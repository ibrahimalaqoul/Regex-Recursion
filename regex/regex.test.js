...
function capitalA(s){
    let regexone = /([a-z]|[A-Z])/g;
    let regextwo = /[!@#$%&()_*\-=\[\]{};':"\\|,.<>\/?]+|\d/g;
    let regexthree = /A$/g;
    let stringone = regexone.test(s);
    let testingfornumsym =regextwo.test(s);
    let endCapA = regexthree.test(s);
    
                        
    if(stringone == true && testingfornumsym == false&& endCapA ==true){
        return true
    }else{
        return false
    }
